...
2. User account the container should run as:
   - non-root user with UID: `$APP_UID` (default non-root user provided by Microsoft base images)
...
- ✅ Configuration of Linux container platform compatibility Azure Linux)
...
# - 10.0-azurelinux3.0 (Azure Linux 3.0, .NET 10 LTS)
# - 9.0-azurelinux3.0 (Azure Linux 3.0, .NET 9 STS)
# - 8.0-azurelinux3.0 (Azure Linux 3.0, .NET 8 LTS)
...
# Possible tags include:
# - 10.0-azurelinux3.0 (Azure Linux 3.0, .NET 10 LTS)
# - 10.0-azurelinux3.0-distroless (Azure Linux 3.0 Distroless, .NET 10 LTS)
# - 10.0-azurelinux3.0-distroless-composite (Azure Linux 3.0 Distroless Composite - optimized, .NET 10 LTS)
# - 9.0-azurelinux3.0 (Azure Linux 3.0, .NET 9 STS)
# - 9.0-azurelinux3.0-distroless (Azure Linux 3.0 Distroless, .NET 9 STS)
# - 8.0-azurelinux3.0 (Azure Linux 3.0, .NET 8 LTS)
# - 8.0-azurelinux3.0-distroless (Azure Linux 3.0 Distroless, .NET 8 LTS)
...
```dockerfile
FROM mcr.microsoft.com/dotnet/aspnet:10.0-azurelinux3.0-distroless AS final
# Distroless images have no package manager, no shell, and non-root by default.
...
```dockerfile
FROM mcr.microsoft.com/dotnet/aspnet:10.0-azurelinux3.0-distroless-composite AS final
# Combines distroless benefits with ReadyToRun (R2R) compilation for optimized startup.
# Available from .NET 8+.
```
...
- Prefer Azure Linux distroless images for production (no shell, no package manager, reduced attack surface)
- Consider Azure Linux distroless composite images for ASP.NET Core apps for optimized startup performance
